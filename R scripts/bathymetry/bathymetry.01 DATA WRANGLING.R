...
# Clip longitudes
H_crop <- which(between(nc_lon, -23, -5)); W <- min(H_crop) ; E <- max(H_crop) # Find max and min index in target window
nc_lon <-nc_lon[c(W:E)]                                         # Clip to window

# Clip latitudes
V_crop <- which(between(nc_lat, 10, 38)); S <- min(V_crop); N <- max(V_crop)  # Find max and min index in target window
