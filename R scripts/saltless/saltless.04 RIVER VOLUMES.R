...
packages <- c("MiMeMo.tools", "furrr", "raster", "data.table")              # List packages
...
domains <- readRDS("./Objects/Domains.rds") %>% st_make_valid()             # Import domain polygon
...
ggplot() +
   ...
   theme_minimal() +
   box_zoom(st_buffer(domains, 500000)) +
