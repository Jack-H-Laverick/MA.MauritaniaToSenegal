...
Boundary_template <- read.csv(stringr::str_glue("./StrathE2E/{implementation}/2010-2019/Driving/chemistry_BRAZILIAN_SHELF_2010-2019.csv"))  # Read in example boundary drivers
...
My_boundary_data<- readRDS("./Objects/Boundary measurements.rds") %>% # Import data
  ...
  summarise(across(SI_NO3:D_phyt, ~ mean(.x, na.rm = T))) %>% 
  ungroup() %>% 
