---
title: "Choosing StrathE2E domain extent - Mauritania/Senegal"
...
**Following our first case study meeting 70 m was suggested to separate the shallow and deep layers based on the euphotic layer depth (https://doi.org/10.5194/bg-2020-25). **
...
**Following our first case study meeting a 900 m maximum depth for the offshore zone was suggested. This is the maximum depth which hake trawlers operate to. **

Finally we need to "cut" the polygons defined by depths and distances from shore so that they don't extend around the whole world. Our first suggestion is to use the combined EEZ for Mauritania, the Gambia, and Senegal. It was originally suggested to start the implementation at 20N but this would have resulted in an overlap with the western Sahara implementation.
...
ggplot() +
  ...
  coord_sf(xlim = c(-19, -13), ylim = c(10,24)) + # Specify the plotting window for SF maps in this region  
  theme_minimal() +
...
The model domain is shown in grey with white borders, while the fishing effort is shown as a heatmap on a log scale. The 900m depth contour does seem to capture the bulk of the fishing activity.